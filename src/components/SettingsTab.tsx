...
export default function SettingsTab({
  ...
  // Profile Photo, Google Login, and Sync states
  ...
  const [adminTarget, setAdminTarget] = useState<'all' | 'current' | 'selected'>('all');
  ...
  const fetchRegisteredUsers = async () => {
    ...
    try {
      ...
      querySnapshot.forEach((docSnap) => {
        ...
        const userEmail = data.profile?.email || (docSnap.id.includes('@') ? docSnap.id : '');
        const userName = data.profile?.name || 'Student';
        const userFcmToken = data.fcmToken || data.profile?.fcmToken || '';
        
        // Avoid duplicate entries (by email) in the display list
      ...
      console.error('Error fetching registered users for admin:', err);
    } finally {
  ...
  useEffect(() => {
    const unsubscribe = auth ? auth.onAuthStateChanged(async (user: any) => {
      if (user) {
        ...
        try {
          ...
          if (docSnap.exists()) {
            ...
            setProfile({
              ...profile,
              photoURL: cloudData?.photoURL || cloudProfile?.photoURL || profile.photoURL || '',
              fcmToken: cloudData?.fcmToken || cloudProfile?.fcmToken || profile.fcmToken || '',
              googleConnected: true,
              email: user.email || profile.email,
              name: user.displayName || profile.name,
              joinedDate: profile.joinedDate || new Date().toLocaleDateString(),
              lastSync: cloudData?.lastSync || cloudProfile?.lastSync || new Date().toISOString()
            });
            
            // Automatically get and save FCM token if permission already granted
            if (Notification.permission === 'granted') {
              registerFCM(user.uid);
            }
          } else {
            // First time Google Sign In - update local profile & sync
          ...
          console.error("Error checking auth state custom updates:", err);
        }
      }
    }) : () => {};
    return () => unsubscribe();
  ...
  const handlePhotoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!currentUser) {
      alert("Please connect your Google Account first to upload your profile photo to Firebase cloud storage.");
      return;
    }
    ...
    try {
      ...
      const downloadURL = await uploadProfilePhoto(currentUser.uid, compressedBlob);
  ...
  const registerFCM = async (uid: string) => {
    try {
      ...
      const token = await getToken(messaging, {
        ...
        vapidKey: 'BM3Fx85v_A18XmG_6hVqF87u0GgY925l_411120043089'
      });
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (user) {
        ...
        // Sync to Firestore
        await setDoc(doc(db, 'users', user.uid), { profile: updatedProfile }, { merge: true });
        await setDoc(doc(db, 'users', (user.email || '').toLowerCase().trim()), { profile: updatedProfile }, { merge: true });
        
        // Get token if permission exists or trigger request
      ...
      console.error(err);
      setAuthError(err.message || 'Google account connection failed.');
    } finally {
  ...
  const handleSendNotification = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (adminTarget === 'all') {
        // Query all users to collect active tokens
        const querySnapshot = await getDocs(collection(db, 'users'));
        querySnapshot.forEach((docSnap) => {
          const data = docSnap.data();
          const t = data.fcmToken || data.profile?.fcmToken;
          if (t && !tokens.includes(t)) tokens.push(t);
        });
      } else if (adminTarget === 'current') {
        const currentToken = profile.fcmToken;
        if (!currentToken) throw new Error("No active FCM Token found for current user. Please ensure notifications are enabled.");
        tokens = [currentToken];
      } else if (adminTarget === 'selected') {
        if (selectedAdminEmails.length === 0) throw new Error("Please select at least one target user.");
        // Map selected emails to their tokens
        selectedAdminEmails.forEach((email) => {
          const userObj = registeredUsers.find(u => u.email.toLowerCase() === email.toLowerCase());
          if (userObj && userObj.fcmToken) {
            tokens.push(userObj.fcmToken);
          }
        });
        if (tokens.length === 0) throw new Error("None of the selected users have active FCM tokens configured.");
      }
      ...
      if (resData.success) {
        setNotificationSuccess(`Successfully sent push notifications! Success count: ${resData.successCount}, Failures: ${resData.failureCount}`);
        setAdminTitle('');
        setAdminMessage('');
      } else {
        throw new Error(resData.error || "Internal server error occurred while sending notification.");
      }
    } catch (err: any) {
      console.error(err);
      setNotificationError(err.message || "Failed to send notifications.");
    } finally {
  ...
  return (
    <div className="space-y-6 pb-24 text-zinc-100">
      ...
      {/* 1. COMPREHENSIVE PROFILE DETAILS CARD */}
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 space-y-5 shadow-xl">
        <div className="flex flex-col sm:flex-row items-center gap-5">
          {/* Avatar & Camera input */}
          <div className="relative group shrink-0">
            <div className="w-24 h-24 rounded-full border-4 border-zinc-950 overflow-hidden bg-zinc-950 flex items-center justify-center shadow-2xl relative">
              {profile.photoURL ? (
                <img 
            ...
            <label 
              ...
              className="absolute bottom-0 right-0 p-2 bg-primary hover:bg-primary/95 text-white rounded-full border-2 border-zinc-900 shadow-lg cursor-pointer transition-transform active:scale-90 flex items-center justify-center"
          ...
          <div className="text-center sm:text-left space-y-1.5 flex-1 w-full min-w-0">
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
              <h2 className="text-xl font-black text-zinc-100 truncate">{profile.name}</h2>
              {profile.googleConnected && (
                <span className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-[9px] font-black uppercase px-2 py-0.5 rounded-full flex items-center gap-1">
                  <CheckCircle2 size={10} /> Verified
                </span>
              )}
            </div>
            <p className="text-xs text-zinc-400 font-medium truncate">{profile.college || 'JMI'}</p>
            
            <div className="flex justify-center sm:justify-start gap-2 mt-2">
              <button
                type="button"
                onClick={() => document.getElementById('profile-photo-input')?.click()}
                className="px-3 py-1.5 bg-zinc-950 border border-zinc-800 text-zinc-300 hover:text-white rounded-xl text-[10px] font-bold uppercase transition-colors flex items-center gap-1.5"
                disabled={isUploading}
              >
                {isUploading ? (
                  <>
        ...
        {/* Upload Success/Error Toast inside Card */}
        {uploadError && (
        ...
        {uploadSuccess && (
          <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-center gap-2 text-xs text-emerald-500 animate-pulse">
        ...
        <div className="grid grid-cols-2 gap-3 border-t border-zinc-800/60 pt-4">
          <div className="bg-zinc-950/40 border border-zinc-850 p-3 rounded-xl space-y-1">
            <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider block">Email Address</span>
            <span className="text-xs text-zinc-300 font-semibold block truncate" title={profile.email}>{profile.email || 'N/A'}</span>
          </div>
          <div className="bg-zinc-950/40 border border-zinc-850 p-3 rounded-xl space-y-1">
            <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider block">Branch / Program</span>
            <span className="text-xs text-zinc-300 font-semibold block truncate">{profile.department || 'N/A'}</span>
          </div>
          <div className="bg-zinc-950/40 border border-zinc-850 p-3 rounded-xl space-y-1">
            <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider block">Current Semester</span>
            <span className="text-xs text-zinc-300 font-semibold block truncate">{profile.semester || 'N/A'}</span>
          </div>
          <div className="bg-zinc-950/40 border border-zinc-850 p-3 rounded-xl space-y-1">
            <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider block">Overall Attendance</span>
            <span className={`text-xs font-black block truncate ${
              stats.percentage >= 75 ? 'text-emerald-500' : 'text-rose-500'
            }`}>
              {stats.percentage ? `${stats.percentage.toFixed(1)}%` : '0.0%'}
            </span>
          </div>
          <div className="bg-zinc-950/40 border border-zinc-850 p-3 rounded-xl space-y-1">
            <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider block">Joined Date</span>
            <span className="text-xs text-zinc-300 font-semibold block truncate">
              {profile.joinedDate || new Date().toLocaleDateString('en-US', { day: '2-digit', month: 'short', year: 'numeric' })}
            </span>
          </div>
          <div className="bg-zinc-950/40 border border-zinc-850 p-3 rounded-xl space-y-1">
            <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider block">Last Server Sync</span>
            <span className="text-xs text-zinc-300 font-semibold block truncate flex items-center gap-1">
              {profile.lastSync ? format(new Date(profile.lastSync), 'dd MMM yyyy, hh:mm a') : 'Never'}
            </span>
          </div>
        </div>

        {/* Google Authentication Integration Status bar */}
        <div className="bg-zinc-950/60 border border-zinc-850/80 p-3 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center shrink-0">
              <Mail size={14} className={profile.googleConnected ? "text-emerald-500" : "text-zinc-500"} />
            </div>
            <div>
              <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider block">Google Account Connection</span>
              <span className="text-xs text-zinc-300 font-bold">
                {profile.googleConnected ? 'Connected & Synced' : 'Not Connected'}
              </span>
            </div>
          </div>
          ...
          <div className="shrink-0 flex gap-2">
            {profile.googleConnected ? (
              <button
                ...
                className="px-3 py-1.5 bg-red-600/10 border border-red-500/20 text-red-500 hover:bg-red-600/20 rounded-xl text-[10px] font-black uppercase transition-colors"
              >
                Disconnect
              </button>
            ) : (
              <button
                ...
                className="px-3 py-1.5 bg-primary text-white hover:bg-primary/90 rounded-xl text-[10px] font-black uppercase transition-colors flex items-center gap-1"
        ...
        {authSuccess && (
          <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-center gap-2 text-xs text-emerald-500 animate-pulse">
            <CheckCircle2 size={14} className="shrink-0" />
            <span>Google account successfully linked to BunkSafe!</span>
          </div>
        )}
      </div>
      ...
      {/* FEATURE 3: ADMIN SEND NOTIFICATION SECTION */}
      <div className="space-y-3">
        <h3 className="text-xs font-bold text-zinc-500 uppercase tracking-widest flex items-center justify-between px-1">
          <span className="flex items-center gap-1.5">
            <Key size={14} className="text-primary" /> Admin Push Panel
          </span>
          <button 
            type="button" 
        ...
        {showAdminPanel && (
          <div className="bg-zinc-900 border border-zinc-800/80 rounded-2xl p-5 space-y-4 shadow-xl">
            <form onSubmit={handleSendNotification} className="space-y-4">
              {/* Target Selector */}
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider block">Target Audience</label>
                <div className="grid grid-cols-3 gap-2">
                  {(['all', 'current', 'selected'] as const).map((t) => (
                    <button
                      key={t}
                      type="button"
                      onClick={() => setAdminTarget(t)}
                      className={`py-2 px-3 border rounded-xl text-xs font-bold uppercase transition-all ${
                        adminTarget === t
                          ? 'bg-primary/10 border-primary text-primary'
                          : 'bg-zinc-950/40 border-zinc-850/80 text-zinc-400 hover:text-zinc-200'
                      }`}
                    >
                      {t === 'all' ? 'All Users' : t === 'current' ? 'Only Me' : 'Select'}
                    </button>
                  ))}
                </div>
              </div>

              {/* Selected Users list (only if target is 'selected') */}
              {adminTarget === 'selected' && (
                <div className="space-y-1.5">
                  <div className="flex justify-between items-center">
                    <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider block">Registered Students</label>
                    <button
                      type="button"
                  ...
                  {isLoadingUsers ? (
                    <div className="bg-zinc-950/55 border border-zinc-850 rounded-xl p-4 flex items-center justify-center gap-2 text-xs text-zinc-500">
                      <Loader2 size={14} className="animate-spin text-primary" /> Loading user registry...
                    </div>
                  ) : registeredUsers.length === 0 ? (
                    <div className="bg-zinc-950/55 border border-zinc-850 rounded-xl p-4 text-center text-xs text-zinc-500">
                      No registered users found.
                    </div>
                  ) : (
                    <div className="bg-zinc-950/55 border border-zinc-850 rounded-xl max-h-48 overflow-y-auto p-2 space-y-1 text-xs">
                      {registeredUsers.map((user) => (
                        ...
                        >
                          <div className="flex items-center gap-2.5">
                            <input 
                              type="checkbox" 
                              checked={selectedAdminEmails.includes(user.email)}
                              onChange={(e) => {
                                if (e.target.checked) {
                                  setSelectedAdminEmails([...selectedAdminEmails, user.email]);
                                } else {
                                  setSelectedAdminEmails(selectedAdminEmails.filter(email => email !== user.email));
                                }
                              }}
                              className="rounded border-zinc-700 bg-zinc-900 text-primary focus:ring-primary/20 w-4 h-4"
                            />
                            <div>
                              <span className="font-bold text-zinc-200 block">{user.name}</span>
                          ...
                          <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full shrink-0 ${
                            ...
                            {user.fcmToken ? 'Push Ready' : 'No Token'}
                          </span>
                        </label>
                      ))}
                    </div>
              ...
              {/* Title Field */}
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider block">Notification Title</label>
                <input
                  type="text"
                  placeholder="e.g. ⚠️ Attendance Reminder"
                  value={adminTitle}
                  onChange={(e) => setAdminTitle(e.target.value)}
                  className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-2.5 text-xs text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:border-primary/50 transition-colors"
                />
              </div>

              {/* Message Field */}
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider block">Message Body</label>
                <textarea
                  rows={3}
                  placeholder="e.g. Please mark today's attendance inside BunkSafe."
                  value={adminMessage}
                  onChange={(e) => setAdminMessage(e.target.value)}
                  className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-2.5 text-xs text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:border-primary/50 transition-colors resize-none"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-3 px-4 bg-primary hover:bg-primary/95 text-white rounded-xl text-xs font-black uppercase flex items-center justify-center gap-2 transition-all active:scale-[0.98] disabled:opacity-55"
              >
                {isSendingNotification ? (
                  ...
                  <>
                    <Send size={14} /> Send Push Notification
                  </>
                )}
              </button>
            </form>
